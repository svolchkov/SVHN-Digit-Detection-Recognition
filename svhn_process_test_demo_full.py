...
for x in [y for y in files if len(y) > 8 and int(y.split('.')[0]) > 14000]:
    #filename = str(x) + ".png"
    filename = x
...
#    if np.all(pre_cropped_pred == correct):  
        ...
        if correct_predictions <= 12:
            ...
            print x," CORRECT"
            print int("".join(['0' if y == 10 else str(y) for y in [x for x in list(result)]]))
            cv2.imshow("Img",img)
            cv2.waitKey(0)
    else:
        ...
        if precropped_incorrect_predictions <= 12:
            ...
            print int("".join(['0' if y == 10 else str(y) for y in [x for x in list(result) if x != 0]]))
            cv2.imshow("Img",img)
            cv2.waitKey(0)
